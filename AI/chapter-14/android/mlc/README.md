...
当前已经跑通的路线是复用 MLC 官方 Android APK 中的 native runtime，再给 `MLCEngineExample` 加 `BenchmarkActivity`。准备 APK、推送模型并压测可以直接执行：
...
`push-model` 会下载并推送 `Qwen2.5-1.5B-Instruct-q4f16_1-MLC` 到 App internal files 目录。
...
`patches/0001-add-mlc-benchmark-activity.patch` 是基于这个 commit 生成和验证的。如果要测试新版 MLC，可以显式传入 `MLC_REF=<commit-or-tag>`，但需要重新确认 patch 是否还能应用。
...
- `android_llm.sh mlc prepare` 已成功构建并安装带 `BenchmarkActivity` 的 debug APK。
- `android_llm.sh mlc push-model` 已成功推送 `Qwen2.5-1.5B-Instruct-q4f16_1-MLC`。
