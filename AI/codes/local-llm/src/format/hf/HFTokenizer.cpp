...
void HFTokenizer::append_utf8(std::string &out, uint32_t cp) {
    if (cp <= 0x7F) {
...
uint32_t HFTokenizer::decode_utf8(const std::string &s, size_t &pos) {
...
void HFTokenizer::build_pre_tokenize_regex() {
...
HFTokenizer::HFTokenizer(const nlohmann::json &tokenizer_json) {
    parse(tokenizer_json);
}

void HFTokenizer::parse(const nlohmann::json &json) {
...
void HFTokenizer::build_byte_unicode_maps() {
...
std::string HFTokenizer::nfc_normalize(const std::string &text) {
...
std::vector<std::string> HFTokenizer::pre_tokenize(const std::string &text) const {
...
std::vector<std::string> HFTokenizer::bpe(const std::string &token) const {
...
std::vector<int> HFTokenizer::encode(const std::string &text) const {
    std::vector<int> ids;
...
void HFTokenizer::debug_dump() const {
