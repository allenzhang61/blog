...
SharedExperts::SharedExperts(const DeepseekLayerWeights &weights, const DeepseekConfig &config, CudaWeightPool *pool)
    : config_(config), lw_(weights), pool_(pool) {}

void SharedExperts::forward(DeepseekSession &session, const float *d_normed, int input_size, float *d_moe) {
    ...
    CudaWeight ffn_gate_shexp = pool_->cached_weight(*lw_.ffn_gate_shexp)->try_dequant();
    GemmInput sgate_in = prepare_gemm_input(d_normed, d_ffn_in_lowp, input_size * hidden_size, ffn_gate_shexp.type, nullptr);
    gemm_weight(pool_->handle, ffn_gate_shexp, sgate_in.ptr, d_gate, shared_ffn, hidden_size, input_size, sgate_in.type, "ds.gemm.sgate");
    CudaWeight ffn_up_shexp = pool_->cached_weight(*lw_.ffn_up_shexp)->try_dequant();
    GemmInput sup_in = prepare_gemm_input(d_normed, d_ffn_in_lowp, input_size * hidden_size, ffn_up_shexp.type, nullptr);
    gemm_weight(pool_->handle, ffn_up_shexp, sup_in.ptr, d_up, shared_ffn, hidden_size, input_size, sup_in.type, "ds.gemm.sup");
    ...
    CudaWeight ffn_down_shexp = pool_->cached_weight(*lw_.ffn_down_shexp)->try_dequant();
    ...
    gemm_weight(pool_->handle, ffn_down_shexp, sdown_in.ptr, d_ffn_out, hidden_size, shared_ffn, input_size, sdown_in.type, "ds.gemm.sdown");
