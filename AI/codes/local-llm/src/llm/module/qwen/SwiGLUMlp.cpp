...
void SwiGLUMlp::forward(QwenSession &session, const float *d_in, float *d_out, size_t rows, int hidden_size) {
    ...
    CudaWeight gate = pool_->cached_weight(weights_.gate_proj)->try_dequant();
    GemmInput in = prepare_gemm_input(d_in, d_input_lowp, rows * static_cast<size_t>(hidden_size), gate.type, nullptr);
    gemm_weight(pool_->handle, gate, in.ptr, d_gate, intermediate, hidden_size, rows, in.type, "mlp.gate");

    CudaWeight up = pool_->cached_weight(weights_.up_proj)->try_dequant();
    ...
    CudaWeight down = pool_->cached_weight(weights_.down_proj)->try_dequant();
