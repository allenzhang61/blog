...
void FullAttention::prefill(QwenSession &session, const GPUTensor &g_hidden, const GPUTensor &g_out) {
    ...
    TensorTool::full_attention_q_batch(g_full_projection, weights_.s_q_norm, g_full_q, g_full_gate,
                                  n_heads, head_dim, start_pos, theta, partial, eps);
    TensorTool::full_attention_kv_batch(g_full_k, g_full_v, weights_.s_k_norm, kv.g_key_cache, kv.g_value_cache,
                                   kv_heads, head_dim, /*max_seq_len=*/0, start_pos, theta, partial, eps);
    TensorTool::full_attention_attend_batch(g_full_q, g_full_gate, kv.g_key_cache, kv.g_value_cache, g_full_attn,
...
void FullAttention::decode(QwenSession &session, const GPUTensor &g_hidden, const GPUTensor &g_out, int pos) {
    ...
    TensorTool::full_attention_kv(g_full_k, g_full_v, weights_.s_k_norm, kv.g_key_cache, kv.g_value_cache,
                             kv_heads, head_dim, /*max_seq_len=*/0, pos, theta, partial, eps);
    TensorTool::full_attention_attend(g_full_q, g_full_gate, kv.g_key_cache, kv.g_value_cache, g_full_attn,
