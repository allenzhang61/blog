//
// Created by zhangyoulun on 9/8/2026.
//

#include "FullAttention.h"

#include <cstddef>
#include <stdexcept>

#include <cuda_runtime.h>

#include "llm/model/qwen/QwenWeights.h"
#include "llm/model/qwen/QwenConfig.h"
#include "llm/model/qwen/QwenForwardScratch.h"
#include "llm/model/qwen/QwenSession.h"
#include "backend/cuda/common.h"
#include "backend/cuda/mem/CudaWeight.h"
#include "backend/cuda/mem/CudaWeightPool.h"
#include "backend/cuda/ops/gemm.h"
#include "backend/cuda/ops/kernel.cuh"

FullAttention::FullAttention(const FullAttnWeights &weights, const TextConfig &config,
                             CudaWeightPool *pool)
    : weights_(weights), config_(config), pool_(pool) {}

void FullAttention::prefill(const float *d_hidden, float *d_out, size_t tokens,
                            FullAttnKVCache &kv, QwenForwardScratch &scratch) {
    const int n_heads = config_.num_attention_heads;
    const int kv_heads = config_.num_key_value_heads;
    const int head_dim = config_.head_dim;
    const int hidden = config_.hidden_size;
    const int q_total = n_heads * head_dim;
    const int kv_total = kv_heads * head_dim;
    const float eps = config_.rms_norm_eps;
    const float theta = config_.rope_parameters.rope_theta;
    const float partial = config_.rope_parameters.partial_rotary_factor;
    const int start_pos = kv.seq_len;
    const int token_count = static_cast<int>(tokens);

    CudaWeight *q_proj_resident = pool_->cached_weight(weights_.q_proj);
    CudaWeight *k_proj_resident = pool_->cached_weight(weights_.k_proj);
    CudaWeight *v_proj_resident = pool_->cached_weight(weights_.v_proj);
    CudaWeight *q_norm_resident = pool_->cached_weight(weights_.q_norm);
    CudaWeight *k_norm_resident = pool_->cached_weight(weights_.k_norm);
    CudaWeight *o_proj_resident = pool_->cached_weight(weights_.o_proj);
    if (!q_proj_resident || !k_proj_resident || !v_proj_resident ||
        !q_norm_resident || !k_norm_resident || !o_proj_resident) {
        throw std::runtime_error("FullAttention 权重上传失败");
    }

    // q_proj 输出 q_total*2（每 head 交错 [q, gate]）；k/v_proj 输出 kv_total。
    float *d_qgate = scratch.full_projection.ensure(tokens * static_cast<size_t>(q_total) * 2, "full qgate");
    float *d_k = scratch.full_k.ensure(tokens * static_cast<size_t>(kv_total), "full k");
    float *d_v = scratch.full_v.ensure(tokens * static_cast<size_t>(kv_total), "full v");
    float *d_q = scratch.full_q.ensure(tokens * static_cast<size_t>(q_total), "full q");
    float *d_g = scratch.full_gate.ensure(tokens * static_cast<size_t>(q_total), "full gate");
    float *d_attn = scratch.full_attn.ensure(tokens * static_cast<size_t>(q_total), "full attn");

    // 输入激活转成权重 dtype（BF16/F16）后再投影。
    uint16_t *d_in_lowp =
        scratch.input_lowp_buffer.ensure(tokens * static_cast<size_t>(hidden), "full in lowp");
    CudaWeight q_proj = q_proj_resident->try_dequant();
    to_weight_lowp(d_hidden, d_in_lowp, tokens * static_cast<size_t>(hidden), q_proj, nullptr);
    gemm_weight(pool_->handle, q_proj, q_total * 2, hidden, d_in_lowp, q_proj.type, tokens, d_qgate, "fullattn.q_proj");

    CudaWeight k_proj = k_proj_resident->try_dequant();
    gemm_weight(pool_->handle, k_proj, kv_total, hidden, d_in_lowp, k_proj.type, tokens, d_k, "fullattn.k_proj");
    CudaWeight v_proj = v_proj_resident->try_dequant();
    gemm_weight(pool_->handle, v_proj, kv_total, hidden, d_in_lowp, v_proj.type, tokens, d_v, "fullattn.v_proj");

    float *key_cache = static_cast<float *>(kv.key_cache.ptr);
    float *value_cache = static_cast<float *>(kv.value_cache.ptr);

    CudaWeight q_norm = q_norm_resident->try_dequant();
    launch_full_attention_q_batch(d_qgate, static_cast<const uint16_t *>(q_norm.ptr), d_q, d_g,
                                  token_count, n_heads, head_dim, start_pos, theta, partial, eps, nullptr);
    CudaWeight k_norm = k_norm_resident->try_dequant();
    launch_full_attention_kv_batch(d_k, d_v, static_cast<const uint16_t *>(k_norm.ptr),
                                   key_cache, value_cache, token_count, kv_heads, head_dim,
                                   /*max_seq_len=*/0, start_pos, theta, partial, eps, nullptr);
    launch_full_attention_attend_batch(d_q, d_g, key_cache, value_cache, d_attn, token_count, n_heads,
                                       kv_heads, head_dim, /*max_seq_len=*/0, start_pos, nullptr);

    // attn 转成 o_proj 权重 dtype 后做输出投影。
    uint16_t *d_attn_lowp =
        scratch.full_attn_lowp.ensure(tokens * static_cast<size_t>(q_total), "full attn lowp");
    CudaWeight o_proj = o_proj_resident->try_dequant();
    to_weight_lowp(d_attn, d_attn_lowp, tokens * static_cast<size_t>(q_total), o_proj, nullptr);
    // o_proj：[hidden, q_total] · attn[q_total, tokens] -> [hidden, tokens]。
    gemm_weight(pool_->handle, o_proj, hidden, q_total, d_attn_lowp, o_proj.type, tokens, d_out, "fullattn.o_proj");

    kv.seq_len += token_count;
    check_cuda(cudaDeviceSynchronize(), "FullAttention prefill 同步失败");
}

void FullAttention::decode(const float *d_hidden, float *d_out, int pos,
                           FullAttnKVCache &kv, QwenForwardScratch &scratch) {
    const int n_heads = config_.num_attention_heads;
    const int kv_heads = config_.num_key_value_heads;
    const int head_dim = config_.head_dim;
    const int hidden = config_.hidden_size;
    const int q_total = n_heads * head_dim;
    const int kv_total = kv_heads * head_dim;
    const float eps = config_.rms_norm_eps;
    const float theta = config_.rope_parameters.rope_theta;
    const float partial = config_.rope_parameters.partial_rotary_factor;

    CudaWeight *q_proj_resident = pool_->cached_weight(weights_.q_proj);
    CudaWeight *k_proj_resident = pool_->cached_weight(weights_.k_proj);
    CudaWeight *v_proj_resident = pool_->cached_weight(weights_.v_proj);
    CudaWeight *q_norm_resident = pool_->cached_weight(weights_.q_norm);
    CudaWeight *k_norm_resident = pool_->cached_weight(weights_.k_norm);
    CudaWeight *o_proj_resident = pool_->cached_weight(weights_.o_proj);
    if (!q_proj_resident || !k_proj_resident || !v_proj_resident ||
        !q_norm_resident || !k_norm_resident || !o_proj_resident) {
        throw std::runtime_error("FullAttention 权重上传失败");
    }

    float *d_qgate = scratch.full_projection.ensure(static_cast<size_t>(q_total) * 2, "full qgate");
    float *d_k = scratch.full_k.ensure(kv_total, "full k");
    float *d_v = scratch.full_v.ensure(kv_total, "full v");
    float *d_q = scratch.full_q.ensure(q_total, "full q");
    float *d_g = scratch.full_gate.ensure(q_total, "full gate");
    float *d_attn = scratch.full_attn.ensure(q_total, "full attn");

    uint16_t *d_in_lowp = scratch.input_lowp_buffer.ensure(hidden, "full in lowp");
    CudaWeight q_proj = q_proj_resident->try_dequant();
    to_weight_lowp(d_hidden, d_in_lowp, hidden, q_proj, nullptr);
    gemm_weight(pool_->handle, q_proj, q_total * 2, hidden, d_in_lowp, q_proj.type, 1, d_qgate, "fullattn.q_proj");

    CudaWeight k_proj = k_proj_resident->try_dequant();
    gemm_weight(pool_->handle, k_proj, kv_total, hidden, d_in_lowp, k_proj.type, 1, d_k, "fullattn.k_proj");
    CudaWeight v_proj = v_proj_resident->try_dequant();
    gemm_weight(pool_->handle, v_proj, kv_total, hidden, d_in_lowp, v_proj.type, 1, d_v, "fullattn.v_proj");

    float *key_cache = static_cast<float *>(kv.key_cache.ptr);
    float *value_cache = static_cast<float *>(kv.value_cache.ptr);

    CudaWeight q_norm = q_norm_resident->try_dequant();
    launch_full_attention_q(d_qgate, static_cast<const uint16_t *>(q_norm.ptr), d_q, d_g,
                            n_heads, head_dim, pos, theta, partial, eps, nullptr);
    CudaWeight k_norm = k_norm_resident->try_dequant();
    launch_full_attention_kv(d_k, d_v, static_cast<const uint16_t *>(k_norm.ptr),
                             key_cache, value_cache, kv_heads, head_dim, /*max_seq_len=*/0, pos,
                             theta, partial, eps, nullptr);
    launch_full_attention_attend(d_q, d_g, key_cache, value_cache, d_attn, n_heads, kv_heads,
                                 head_dim, /*max_seq_len=*/0, pos, nullptr);

    uint16_t *d_attn_lowp = scratch.full_attn_lowp.ensure(q_total, "full attn lowp");
    CudaWeight o_proj = o_proj_resident->try_dequant();
    to_weight_lowp(d_attn, d_attn_lowp, q_total, o_proj, nullptr);

    gemm_weight(pool_->handle, o_proj, hidden, q_total, d_attn_lowp, o_proj.type, 1, d_out, "fullattn.o_proj");

    kv.seq_len = pos + 1;
    check_cuda(cudaDeviceSynchronize(), "FullAttention decode 同步失败");
}
