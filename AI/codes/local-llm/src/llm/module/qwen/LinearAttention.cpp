...
void LinearAttention::prefill(const float *d_hidden, float *d_out, size_t tokens,
    ...
    CudaWeight *qkv_resident = pool_->cached_weight(weights_.in_proj_qkv);
    CudaWeight *z_resident = pool_->cached_weight(weights_.in_proj_z);
    CudaWeight *b_resident = pool_->cached_weight(weights_.in_proj_b);
    CudaWeight *a_resident = pool_->cached_weight(weights_.in_proj_a);
    CudaWeight *conv_resident = pool_->cached_weight(weights_.conv1d);
    CudaWeight *a_log_resident = pool_->cached_weight(weights_.a_log);
    CudaWeight *dt_bias_resident = pool_->cached_weight(weights_.dt_bias);
    CudaWeight *norm_resident = pool_->cached_weight(weights_.norm);
    CudaWeight *out_proj_resident = pool_->cached_weight(weights_.out_proj);
    if (!qkv_resident || !z_resident || !b_resident || !a_resident || !conv_resident ||
        !a_log_resident || !dt_bias_resident || !norm_resident || !out_proj_resident) {
    ...
    CudaWeight qkv = qkv_resident->try_dequant();
    ...
    CudaWeight conv = conv_resident->try_dequant();
    ...
    CudaWeight a_log = a_log_resident->try_dequant();
    CudaWeight dt_bias = dt_bias_resident->try_dequant();
    CudaWeight norm = norm_resident->try_dequant();
    ...
    CudaWeight out_proj = out_proj_resident->try_dequant();
