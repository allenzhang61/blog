...
int QwenModel::prefill(const CPUTensor &input) {
...
int QwenModel::prefill_session(QwenSession &session, const CPUTensor &input) {
    const TextConfig &text_config = config_.data.text;
    ...
    GPUTensor last = GPUTensor::gpu_view(d_last, {1, static_cast<int64_t>(hidden_size)});
...
int QwenModel::decode_session(QwenSession &session, int prev_token_id, int pos) {
    ...
    CPUTensor input_view = CPUTensor::host_view(&token_id, {1}, DType::I32);
