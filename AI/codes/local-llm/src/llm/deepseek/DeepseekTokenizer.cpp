...
void DeepseekTokenizer::append_utf8(std::string &out, uint32_t cp) {
    if (cp <= 0x7F) {
...
uint32_t DeepseekTokenizer::decode_utf8(const std::string &s, size_t &pos) {
...
void DeepseekTokenizer::build_pre_tokenize_regex() {
...
DeepseekTokenizer::DeepseekTokenizer(const GgufFile &gguf) {
    // 1) vocab：tokenizer.ggml.tokens 数组，id = 下标。
    const GgufValue &tokens = gguf.metadata("tokenizer.ggml.tokens");
    if (tokens.type != GgufValueType::ARRAY || tokens.elem_type != GgufValueType::STRING) {
        throw std::runtime_error("tokenizer.ggml.tokens 不是字符串数组");
    }
    for (size_t i = 0; i < tokens.arr_str.size(); ++i) {
        const std::string &tok = tokens.arr_str[i];
    ...
    if (gguf.has_metadata("tokenizer.ggml.token_type")) {
        const GgufValue &tt = gguf.metadata("tokenizer.ggml.token_type");
        for (size_t i = 0; i < tt.arr_i64.size() && i < tokens.arr_str.size(); ++i) {
            const int type = static_cast<int>(tt.arr_i64[i]);
            if (type == 3 || type == 4) {
                special_tokens.emplace(tokens.arr_str[i], static_cast<int>(i));
            }
        }
    }
    ...
    if (gguf.has_metadata("tokenizer.ggml.merges")) {
        const GgufValue &merges = gguf.metadata("tokenizer.ggml.merges");
        int rank = 0;
        for (const std::string &pair : merges.arr_str) {
    ...
    if (gguf.has_metadata("tokenizer.ggml.bos_token_id")) {
        bos_token_id_ = static_cast<int>(gguf.metadata_i64("tokenizer.ggml.bos_token_id"));
    }
    if (gguf.has_metadata("tokenizer.ggml.add_bos_token")) {
        add_bos_ = gguf.metadata_bool("tokenizer.ggml.add_bos_token");
    } else {
...
void DeepseekTokenizer::build_byte_unicode_maps() {
    std::vector<int> bs;
...
std::vector<std::string> DeepseekTokenizer::pre_tokenize(const std::string &text) const {
...
std::vector<std::string> DeepseekTokenizer::bpe(const std::string &token) const {
...
std::vector<int> DeepseekTokenizer::Encode(const std::string &text) const {
    std::vector<int> ids;
...
std::string DeepseekTokenizer::Decode(const std::vector<int> &ids) const {
    std::string mapped;
...
void DeepseekTokenizer::DebugDump() const {
    std::ostringstream out;
    out << "DeepseekTokenizer:\n";
