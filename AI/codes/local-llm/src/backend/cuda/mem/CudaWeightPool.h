...
public:
    ...
    CudaWeight *cached_weight(const TensorView &weight);
...
private:
    ...
    static cudaDataType_t cuda_type_for(const TensorView &weight);

    // 返回当前支持 dtype 的单元素字节数。
    static size_t dtype_size_for(const TensorView &weight);
