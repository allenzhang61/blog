...
public:
    ...
    // 已够大时复用旧内存；name 仅用于分配失败时的错误信息。
    template <typename T>
    T *ensure(const std::string &key, size_t count, const std::string &name) {
        return static_cast<T *>(ensure_bytes(key, count * sizeof(T), name));
    }
...
private:
    ...
    // 确保 key 缓冲至少 required_bytes 字节，返回裸指针。
    void *ensure_bytes(const std::string &key, size_t required_bytes, const std::string &name);

    std::unordered_map<std::string, Buffer> buffers_;
};

// 预定义的 scratch key：qwen / deepseek 共用同一套命名空间。
// 每个 key 对应原 *ForwardScratch 里的一个字段，一字段一 key。
...
inline constexpr const char *kInputLowp = "input_lowp";        // gemm 输入的通用低精度缓冲
...
// ---- qwen mlp / norm / 残差 / 采样 ----
inline constexpr const char *kYBuffer = "y_buffer";            // 通用输出 / logits
inline constexpr const char *kGateBuffer = "gate_buffer";      // mlp gate
inline constexpr const char *kUpBuffer = "up_buffer";          // mlp up
inline constexpr const char *kProdBuffer = "prod_buffer";      // SiLU(gate)*up
inline constexpr const char *kProdLowp = "prod_lowp";          // prod 低精度
inline constexpr const char *kMixerBuffer = "mixer_buffer";    // attention / mixer 子层输出
...
inline constexpr const char *kGate = "gate";                   // [tokens, ffn]
inline constexpr const char *kUp = "up";                       // [tokens, ffn]
...
// ---- deepseek logits ----
inline constexpr const char *kLogits = "logits";               // [vocab]
inline constexpr const char *kLogitsInLowp = "logits_in_lowp";
