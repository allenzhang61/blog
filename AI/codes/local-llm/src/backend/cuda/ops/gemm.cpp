...
void gemm_weight(cublasHandle_t handle, const GPUTensor &g_weight,
                 ...
                 const char *name) {
    const cudaDataType_t weight_type = cuda_type_of(g_weight.dtype);
    ...
    // name 非空时埋点：以 g_weight.bytes 作为访存字节，供 Profiler 算有效带宽。
    // ScopedGpuTimer 在 Profiler 关闭或 name 为空时零开销。
    ScopedGpuTimer timer(name && name[0] ? name : std::string(), nullptr, g_weight.nbytes);
