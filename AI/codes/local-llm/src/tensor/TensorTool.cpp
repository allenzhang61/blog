...
void TensorTool::add(const GPUTensor &g_a, const GPUTensor &g_b, const GPUTensor &g_out, void *stream) {
    launch_add(g_a.data<float>(), g_b.data<float>(), g_out.data<float>(), static_cast<int>(g_out.numel()), stream);
}
...
void TensorTool::full_attention_kv(const GPUTensor &g_k_in, const GPUTensor &g_v_in,
                                   ...
                                   float partial_rotary_factor, float eps, void *stream) {
    launch_full_attention_kv(g_k_in.data<float>(), g_v_in.data<float>(), lowp_data(s_k_norm_weight),
                             g_key_cache.data<float>(), g_value_cache.data<float>(), kv_heads,
...
void TensorTool::full_attention_kv_batch(const GPUTensor &g_k_in, const GPUTensor &g_v_in,
    ...
    launch_full_attention_kv_batch(g_k_in.data<float>(), g_v_in.data<float>(), lowp_data(s_k_norm_weight),
                                   g_key_cache.data<float>(), g_value_cache.data<float>(),
...
void TensorTool::full_attention_attend(const GPUTensor &g_q, const GPUTensor &g_gate,
    ...
    launch_full_attention_attend(g_q.data<float>(), g_gate.data<float>(), g_key_cache.data<float>(),
                                 g_value_cache.data<float>(), g_attn.data<float>(), n_heads, kv_heads,
...
void TensorTool::full_attention_attend_batch(const GPUTensor &g_q, const GPUTensor &g_gate,
                                             ...
                                             int head_dim, int max_seq_len, int start_pos,
                                             void *stream) {
    launch_full_attention_attend_batch(g_q.data<float>(), g_gate.data<float>(), g_key_cache.data<float>(),
                                       g_value_cache.data<float>(), g_attn.data<float>(), static_cast<int>(g_q.rows()),
...
void TensorTool::linear_attention_conv(const GPUTensor &g_mixed, const StorageTensor &s_conv_weight,
    ...
    launch_linear_attention_conv(g_mixed.data<float>(), lowp_data(s_conv_weight), g_conv_state.data<float>(),
                                 g_conv_out.data<float>(), static_cast<int>(g_mixed.cols()),
                                 kernel, stream);
}
...
void TensorTool::linear_attention_conv_batch(const GPUTensor &g_mixed, const StorageTensor &s_conv_weight,
                                             const GPUTensor &g_conv_state, const GPUTensor &g_conv_out,
                                             int kernel, void *stream) {
    launch_linear_attention_conv_batch(g_mixed.data<float>(), lowp_data(s_conv_weight),
                                       g_conv_state.data<float>(), g_conv_out.data<float>(),
...
void TensorTool::linear_attention_recurrent(const GPUTensor &g_conv_out, const GPUTensor &g_z,
    ...
    launch_linear_attention_recurrent(g_conv_out.data<float>(), g_z.data<float>(), g_b.data<float>(), g_a.data<float>(),
                                      f32_data(s_a_log), lowp_data(s_dt_bias), f32_data(s_norm_weight),
                                      g_recurrent_state.data<float>(), g_gated.data<float>(), key_heads, value_heads,
...
void TensorTool::mla_kv_a(const GPUTensor &g_kv_a, const StorageTensor &s_kv_a_norm_weight,
    ...
    launch_mla_kv_a(g_kv_a.data<float>(), f32_data(s_kv_a_norm_weight), g_kv_cache.data<float>(),
                    kv_lora, qk_rope, max_seq_len, pos, g_inv_freq.data<float>(), eps, stream);
}
...
void TensorTool::mla_kv_a_batch(const GPUTensor &g_kv_a, const StorageTensor &s_kv_a_norm_weight,
                                ...
                                float eps, void *stream) {
    launch_mla_kv_a_batch(g_kv_a.data<float>(), f32_data(s_kv_a_norm_weight), g_kv_cache.data<float>(),
...
void TensorTool::mla_attend(const GPUTensor &g_q, const GPUTensor &g_kv_b_out, const GPUTensor &g_kv_cache,
    ...
    launch_mla_attend(g_q.data<float>(), g_kv_b_out.data<float>(), g_kv_cache.data<float>(), g_attn.data<float>(),
...
void TensorTool::mla_attend_batch(const GPUTensor &g_q, const GPUTensor &g_kv_b_out,
                                  ...
                                  float softmax_scale, void *stream) {
    launch_mla_attend_batch(g_q.data<float>(), g_kv_b_out.data<float>(), g_kv_cache.data<float>(), g_attn.data<float>(),
...
void TensorTool::moe_router_topk(const GPUTensor &g_router_logits, const GPUTensor &g_top_idx, const GPUTensor &g_top_w,
                                 ...
                                 void *stream) {
    launch_moe_router_topk(g_router_logits.data<float>(), g_top_idx.data<int>(), g_top_w.data<float>(),
...
void TensorTool::moe_accumulate(const GPUTensor &g_expert_out, float weight, const GPUTensor &g_out,
                                void *stream) {
    launch_moe_accumulate(g_expert_out.data<float>(), weight, g_out.data<float>(),
