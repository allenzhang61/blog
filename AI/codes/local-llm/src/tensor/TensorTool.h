...
public:
    ...
    static void silu_mul(const GPUTensor &g_gate, const GPUTensor &g_up, const GPUTensor &g_out, void *stream = nullptr);

    static void full_attention_q(const GPUTensor &g_q_and_gate, const StorageTensor &s_q_norm_weight,
                                 ...
                                 void *stream = nullptr);
    static void full_attention_q_batch(const GPUTensor &g_q_and_gate, const StorageTensor &s_q_norm_weight,
                                       const GPUTensor &g_q, const GPUTensor &g_gate, int n_heads, int head_dim,
                                       int start_pos, float rope_theta, float partial_rotary_factor,
                                       float eps, void *stream = nullptr);
    static void full_attention_kv(const GPUTensor &g_k_in, const GPUTensor &g_v_in, const StorageTensor &s_k_norm_weight,
                                  const GPUTensor &g_key_cache, const GPUTensor &g_value_cache, int kv_heads, int head_dim,
                                  int max_seq_len, int pos, float rope_theta,
                                  float partial_rotary_factor, float eps, void *stream = nullptr);
    static void full_attention_kv_batch(const GPUTensor &g_k_in, const GPUTensor &g_v_in,
                                        ...
                                        void *stream = nullptr);
    static void full_attention_attend(const GPUTensor &g_q, const GPUTensor &g_gate, const GPUTensor &g_key_cache,
                                      const GPUTensor &g_value_cache, const GPUTensor &g_attn, int n_heads,
                                      int kv_heads, int head_dim, int max_seq_len, int pos,
                                      void *stream = nullptr);
    static void full_attention_attend_batch(const GPUTensor &g_q, const GPUTensor &g_gate,
    ...
    static void linear_attention_conv(const GPUTensor &g_mixed, const StorageTensor &s_conv_weight,
                                      const GPUTensor &g_conv_state, const GPUTensor &g_conv_out,
                                      int kernel, void *stream = nullptr);
    static void linear_attention_conv_batch(const GPUTensor &g_mixed, const StorageTensor &s_conv_weight,
                                            const GPUTensor &g_conv_state, const GPUTensor &g_conv_out,
                                            int kernel, void *stream = nullptr);
    static void linear_attention_recurrent(const GPUTensor &g_conv_out, const GPUTensor &g_z, const GPUTensor &g_b,
                                           const GPUTensor &g_a, const StorageTensor &s_a_log,
                                           const StorageTensor &s_dt_bias, const StorageTensor &s_norm_weight,
                                           const GPUTensor &g_recurrent_state, const GPUTensor &g_gated, int key_heads,
                                           int value_heads, int k_dim, int v_dim, float eps,
                                           void *stream = nullptr);
    static void linear_attention_recurrent_batch(const GPUTensor &g_conv_out, const GPUTensor &g_z,
                                                 ...
                                                 const GPUTensor &g_gated, int key_heads,
                                                 int value_heads, int k_dim, int v_dim, float eps,
                                                 void *stream = nullptr);
