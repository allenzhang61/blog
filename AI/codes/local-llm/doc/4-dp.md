...
最新 DeepSeek 默认路径已切换为 quant-direct decode：decode GEMV 走 `quant_direct_gemm()`，MoE routed decode 走 device-side indexed 路径，CUDA Graph 默认启用；如需回退到 correctness-safe dequant 路径，可显式设置 `LOCAL_LLM_DEEPSEEK_QUANT_DIRECT=0`。
...
针对 P1 调研定位的根本约束，重构了 GEMM 与 Embedding 的权重访问路径，探索从「量化权重 → 反量化成 F16 → cuBLAS」改为**「量化权重常驻 device + 量化直算 kernel（on-the-fly 解块）」**。DeepSeek 的 greedy 输出对逐层数值漂移非常敏感，因此当前默认策略收敛为：**Embedding 保持量化查表；DeepSeek decode 默认 quant-direct；prefill GEMM 仍保持 correctness-safe dequant+cuBLAS；如需关闭 decode quant-direct，可显式设置 `LOCAL_LLM_DEEPSEEK_QUANT_DIRECT=0`**。
...
- 入口 [`TensorTool::gemm`](../src/tensor/TensorTool.cpp) 对量化权重支持直算；DeepSeek `ds.gemm.*` 在 decode GEMV（`m == 1`）默认走 `quant_direct_gemm()`，显式 `LOCAL_LLM_DEEPSEEK_QUANT_DIRECT=0` 时回退 safe path，prefill 保持 safe path。
...
| shared experts `sdown_add` fused-add（quant-direct 模式） | **~63.2–63.5 t/s** | **~17.4x** | shared expert 的 down projection 直接从 F32 act 做 quant GEMV 并累加到 MoE 输出，跳过 `g_ffn_out` 临时和单独 add；64-token profile 中 shared experts 约 `3.0 ms/token -> 2.49 ms/token`，12/12 单测通过 |
| quantized latent MLA absorption（实验 correctness-first） | smoke **~61.5 t/s** | - | 每层新增 raw Q8_1 latent cache，prefill/decode 按 expanded `kv_b` 相同 Q8_1 header 语义写入；absorption attention 读取 Q8_1 latent，并补 Q4_K raw-sum `min*xsum` 校正；France smoke 恢复 `The capital of France is Paris.`，12/12 单测通过。当前 kernel 仍是朴素实现，性能不作为有效收益 |
| hybrid MLA absorption（额外实验开关） | **~56.8 t/s**（同 prompt profile） | - | `LOCAL_LLM_EXPERIMENTAL_DEEPSEEK_MLA_ABSORB_HYBRID=1`；K-side 用 quantized latent absorption 算 score，V-side 只投影当前 token 的 V rows 并复用 expanded V cache。`ds.decode.mla_forward` 从 expanded **233 ms/64 tok** 降到 **162 ms/64 tok**，但 greedy 输出轨迹仍会因微小差异改变，暂不默认启用 |
| routed down Q8_1 shared staging（实验开关） | **58.0 t/s**（低于默认 **62.6 t/s**） | - | `LOCAL_LLM_EXPERIMENTAL_FUSED_MOE_DOWN=1`；act 仍只量化一次为 Q8_1，down kernel 内把每个 route/qblk 的 Q8_1 block 放到 shared memory，让多条 hidden row warp 复用。Q8_0/Q4_K/Q6_K 对拍通过，但 profile 仍为负收益，默认关闭 |
| routed e_swiglu 实验优化 | **无有效收益** | - | 试过 Q8_1 activation SwiGLU、fast `__expf` SiLU、block-level F32 解块三条路径。Q8_1 与 block-level 会让 `ds.gemm.e_indexed_moe` 变慢并改变输出轨迹；fast `__expf` 对平均耗时几乎无影响。均默认关闭 |
...
CUDA Graph 后续可选：chunked graph decode，目标是减少每 token 末尾的 `d_token -> host` D2H 同步，以及下一 token 的 host `token/pos -> device` H2D。当前 `decode()` 接口每步返回 host token，所以为了 EOS 判断和 `append_output()` 必须每 token 回读一次。可行演进是新增 `decode_many()`：graph 内用 device 端 `d_pos/d_step` 自增，写入 `d_output_tokens[max_output_tokens]`，host 每 `chunk_size=16/32` 步回读一段 token 或 `d_eos_seen`；benchmark 场景可令 `chunk_size=max_output_tokens`，最后一次性回读并按 EOS 截断。该方案只适合 greedy/device-argmax 路径，预计收益约 **2%–5%**；若要完全 device-side EOS 停止，则需要 CUDA Graph conditional node 或 persistent-kernel 风格，暂不优先。

后续收益排序：第一优先是 shared experts 专项融合，profile 中 `ds.gemm.s_swiglu + ds.gemm.sdown` 仍约 **3 ms/token**，可做固定 2 shared experts 的 `gate/up/SwiGLU -> Q8_1 -> down -> accumulate` 专用路径，预期 **5%–10%**；第二优先是 routed MoE 继续减少中间写回，把 `quant_swiglu_indexed -> quantize_q8_1 -> quant_down_q8_1_indexed_accum` 中的 activation 写回和量化 kernel 合并，预期 **5%–8%**，但数值/寄存器压力风险更高；第三优先是 `lm_head` 的 quant GEMV + argmax 融合，当前约 **1.7 ms/token**，避免完整 logits 落全局内存后再 argmax，预期 **4%–7%**；第四优先才是 chunked graph decode，主要减少 host 同步，预期 **2%–5%**。

2026-08 新一轮对照：同机同模型下，local-llm quant-direct + CUDA Graph `tg128` 约 **61.2–61.5 t/s**，llama.cpp `llama-bench -p 27 -n 128 -r 3` 约 **231.5 t/s**，仍有约 **3.8x** 差距。local-llm eager profile 中 routed MoE 约 **5.3 ms/token**、shared experts 约 **3.0 ms/token**、MLA 约 **3.6 ms/token**、`lm_head` 约 **1.7 ms/token**；其中 routed+shared MoE 已超过 llama.cpp 整个 token 的约 **4.3 ms/token**。因此下一阶段需要优先看结构性优化，而不是只做 host 同步收尾。

新的大项优先级：第一优先改为 **MLA matrix absorption**，llama.cpp DeepSeek2 graph 使用 absorption，把 `wk_b` 吸收到 Q/attention/WO 流程中，避免 local-llm 当前的 expanded `kv_b` cache、`kv_b` 当前 token 投影、按 `n_heads` 展开的 `mla_attend_batch` 和独立 `attn_output` 路径；这比单纯缓存 `kv_b` 更接近 llama.cpp 的计算图，尤其长上下文收益更明显。第二优先是 **MoE 专用 fused/直算路径**：shared experts 只有固定 2 个 expert，当前 `quant_swiglu_kernel` ncu 显示 grid 偏小、occupancy 约 **38%**；routed MoE 虽然 down 已优化到约 **65us**，但仍有 `SwiGLU F32 activation -> Q8_1 quantize -> down` 的中间写回。第三优先是 **量化 GEMV 内核追平 llama.cpp 的 `mul_mat_vec_q`**，llama.cpp 代表 kernel 有多个 **4–7us** 级别的小投影，而 local-llm 的若干 Q4/Q8 GEMV 仍在 **15–200us** 区间。第四优先是 **`lm_head` fused GEMV+argmax**；第五优先才是 chunked graph decode。

P0 MLA absorption 复核：当前使用的 GGUF 只有 `blk.0.attn_kv_b.weight`，没有 split `attn_k_b.weight/attn_v_b.weight`；local-llm 只能从 unsplit `wkv_b` 上按布局手工派生 `wk_b/wv_b`。已做 `LOCAL_LLM_EXPERIMENTAL_DEEPSEEK_MLA_ABSORB=1` 的最小 decode 原型，路径为 `q_nope -> latent Q`、latent KV attention、latent context -> V head，再复用 `attn_output`；该原型能运行且短测约 **67 t/s**，但 France smoke 输出变成无关 token，正确性未通过，因此保持实验开关关闭，不能作为有效 P0 合入。后续若继续做 P0，应先增加对 unsplit `wkv_b` 的离线/启动期 split-layout 对拍测试，逐层比较 expanded 路径与 absorption 路径的 `q_abs/kq/attn/attn_out`，再考虑性能优化。

MLA absorption 对拍工具：新增 `LOCAL_LLM_DEBUG_DEEPSEEK_MLA_ABSORB_COMPARE=1`，默认只在 expanded 正常路径下额外计算一份 absorption 中间量，不改变主输出；`LOCAL_LLM_DEBUG_DEEPSEEK_MLA_ABSORB_LAYER=<layer>` 指定层，`LOCAL_LLM_DEBUG_DEEPSEEK_MLA_ABSORB_REPORTS=<n>` 控制输出次数。对拍项包括 `q_abs` 统计、`kq score` expanded-vs-absorb、`softmax` expanded-vs-absorb、`attn_latent` kernel-vs-CPU、`attn_ctx` expanded-vs-absorb、`attn_out` expanded-vs-absorb。第一版 raw-F32 latent absorption 抽查 layer 0/1/8/16/26 的第一个 decode token，`kq_score max_abs` 约 **0.02–0.045**，`softmax max_abs` 约 **0.001–0.0023**，`attn_out max_abs` 到末层约 **0.06**；说明 unsplit `wkv_b` 的 head-major 解析基本正确，主要差异更像来自 expanded quant-direct 路径对 latent 先做 Q8_1 量化，而 absorption 原型直接用 raw F32 latent，二者不严格可交换。

quantized latent absorption 修正：为每层增加 raw byte Q8_1 latent cache，而不是把压缩行格式塞进 dense `GPUTensor`；prefill 复用 expanded `kv_b` 的 Q8_1 语义，Q4_K/Q6_K MMQ prefill 写 raw `sum(x)`，decode 写 `d * sum(qs)`。absorption attention 读取 Q8_1 latent 的 dequant 值，同时对 Q4_K 的 `-dmin * min * xsum` 做 per-head/per-block 校正：K-side score 使用 `q_abs_xsum_delta * (stored_xsum - d*sum(qs))`，V-side projection 使用 attention 加权后的 `attn_xsum_delta` 补同一项。修正后同机同 prompt 抽查 layer 0/1/8/16/26 的第一个 decode token：`kq_score max_abs` 约 **1.9e-6–5.3e-6**，`softmax max_abs` 约 **1.8e-7–6.6e-7**，`attn_ctx max_abs` 约 **8.0e-8–5.4e-7**；`attn_out max_abs` 约 **6.0e-4–3.4e-3**，主要来自 `attn_output` 后续 Q8_1 动态量化对极小 `attn_ctx` 差异的放大。`LOCAL_LLM_EXPERIMENTAL_DEEPSEEK_MLA_ABSORB=1` 下 France smoke 恢复输出 `The capital of France is Paris.`；当前实现仍是 correctness-first 的朴素 kernel，短 smoke 约 **61.5 t/s**，还不能作为性能优化默认打开。

absorption 性能轮廓：64-token 技术长文 prompt 下，expanded 路径 `ds.decode.mla_forward` 约 **233 ms**，其中 `mla_attend_batch` 约 **67 ms**、`ds.gemm.kv_b` 约 **41 ms**。full absorption 虽省掉 decode 的 `kv_b`，但需要计算每 head 的 512 维 `attn_latent` 再做 V projection，`mla_absorb_context` 曾达到约 **97 ms**，整体不划算。新增两段式 attention 后，score 被拆成 `mla_absorb_score`，用一 warp 处理一个 `(head, token)`，64-token 下约 **11.5 ms**；随后新增 hybrid 路径，让 V-side 复用 expanded V cache，并把 V-only projection 改成 Q4_K/Q8_1 block dot 后，`mla_project_v` 从约 **57.9 ms** 降到 **21.1 ms**，`mla_absorb_context_v` 约 **42.8 ms**，`ds.decode.mla_forward` 降到约 **161.9 ms**。不过该路径的 France smoke 仍可能输出 `The question is the capital of France.` 这类非目标答案，说明 greedy 轨迹仍对 `attn_out ~1e-3` 级别差异敏感；因此 hybrid 仅作为 `LOCAL_LLM_EXPERIMENTAL_DEEPSEEK_MLA_ABSORB_HYBRID=1` 性能实验，不随 `LOCAL_LLM_EXPERIMENTAL_DEEPSEEK_MLA_ABSORB=1` 默认打开。
...
- 命令：DeepSeek 量化直通 decode 已是默认策略；`LOCAL_LLM_DEEPSEEK_QUANT_DIRECT=0` 可显式关闭。当前 generic `quant_direct_gemm()` 已回退为 F32 activation + on-the-fly weight dequant，不再包含 decode Q8_1 activation 分支。prefill quant-direct 及其 exclude ops 开关已移除。
...
- decode indexed MoE：旧 device-indexed MoE 实验开关已移除，decode `input_size == 1` 默认进入 `TensorTool::moe_routed_decode_indexed()`。该入口现在是 graph-safe 的 correctness-first device-side 实现：`top_idx/top_w` 均保留在 device，先用 `launch_quant_swiglu_indexed()` 生成 F32 `act[k, ffn]`，再用 `launch_quant_down_f32_indexed_accum_ordered()` 按 route 顺序做 F32 act + on-the-fly weight dequant 累加，避免旧 Q8_1/atomic fused indexed kernel 导致的 greedy 重复退化。
...
- 测试：211 上 `local_llm` / `local_llm_tests` 构建通过；`ctest` 为 16/16；新增 `LaunchQuantizeQ81Test.SupportsRawAndQuantizedBlockSums` 覆盖 Q8_1 header 的 `d * sum(qs)` 与 raw `sum(x)` 两种写法，新增 Q4_K/Q6_K × Q8_1 focused 单测锁住 K-quant block dot 公式，新增 Q8_0/Q4_K/Q6_K routed fused down 对拍单测，并新增 indexed SwiGLU Q8_1 / block-level F32 对拍单测；DeepSeek story smoke 在单独打开 `LOCAL_LLM_DEEPSEEK_QUANT_DIRECT=1` 下可跑完并输出连贯英文故事。
