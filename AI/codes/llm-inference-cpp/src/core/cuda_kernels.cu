...
__global__ void silu_mul_gate_up_batch_kernel(const float * gate_up, float * out, int tokens, int intermediate) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    const int total = tokens * intermediate;
    if (i >= total) {
        return;
    }
    const int tok = i / intermediate;
    const int col = i - tok * intermediate;
    const float * row = gate_up + static_cast<size_t>(tok) * intermediate * 2;
    const float g = row[col];
    const float u = row[intermediate + col];
...
__global__ void float_to_f16_kernel(const float * input, uint16_t * output, int n) {
    ...
    output[i] = reinterpret_cast<const uint16_t *>(&h)[0];
}

__global__ void lowp_row_to_float_kernel(const uint16_t * input, float * output, int n, int lowp_type) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) {
        return;
    }
    if (lowp_type == 1) {
        const __half h = reinterpret_cast<const __half *>(input)[i];
        output[i] = __half2float(h);
    } else {
        output[i] = bf16_bits_to_float(input[i]);
    }
}

__global__ void lowp_embedding_id_to_float_kernel(
    ...
    const uint16_t * row = input + static_cast<size_t>(token) * hidden;
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= hidden) {
        return;
    }
    if (lowp_type == 1) {
        const __half h = reinterpret_cast<const __half *>(row)[i];
        output[i] = __half2float(h);
    } else {
        output[i] = bf16_bits_to_float(row[i]);
    }
}

__global__ void embedding_batch_to_float_kernel(
    const uint16_t * emb,
    const int * token_ids,
    float * output,
    int tokens,
    ...
    const uint16_t * row = emb + static_cast<size_t>(token) * hidden;
    if (lowp_type == 1) {
        const __half hv = reinterpret_cast<const __half *>(row)[h];
        output[i] = __half2float(hv);
    } else {
        output[i] = bf16_bits_to_float(row[h]);
    }
}

__global__ void copy_int_kernel(const int * input, int * output) {
    *output = *input;
}

__global__ void rms_norm_batch_to_bf16_kernel(
    const float * input,
    ...
    const float * row = input + static_cast<size_t>(t) * hidden;
    uint16_t * out = output + static_cast<size_t>(t) * hidden;
    float ss = 0.0f;
    ...
    const float scale = rsqrtf(partial[0] / static_cast<float>(hidden) + eps);
    for (int i = tid; i < hidden; i += blockDim.x) {
        const float w = bf16_bits_to_float(weight[i]);
        const float factor = one_plus ? (1.0f + w) : w;
        out[i] = float_to_bf16_bits(row[i] * scale * factor);
    }
}
...
__global__ void rms_norm_to_f16_kernel(
    ...
    for (int i = tid; i < n; i += blockDim.x) {
        const float w = bf16_bits_to_float(weight[i]);
        const float factor = one_plus ? (1.0f + w) : w;
        const __half h = __float2half_rn(input[i] * scale * factor);
        output[i] = reinterpret_cast<const uint16_t *>(&h)[0];
    }
}
...
__global__ void linear_attention_conv_batch_kernel(
    const float * mixed,
    const uint16_t * conv_weight,
    float * conv_state,
    float * conv_out,
    ...
    for (int t = 0; t < tokens; ++t) {
        ...
        conv_out[static_cast<size_t>(t) * conv_dim + d] = sum / (1.0f + expf(-sum));
    }
}
...
__global__ void linear_attention_recurrent_batch_kernel(
    const float * conv_out,
    const float * z,
    const float * b,
    const float * a,
    ...
    for (int tok = 0; tok < tokens; ++tok) {
        const float * token_conv = conv_out + static_cast<size_t>(tok) * (key_total * 2 + value_total);
        const float * query_base = token_conv;
        const float * key_base = token_conv + key_total;
        const float * value_base = token_conv + key_total * 2;
        ...
        const float beta = 1.0f / (1.0f + expf(-b_base[vh]));
        const float dt = a_base[vh] + bf16_bits_to_float(dt_bias[vh]);
        const float softplus_dt = dt > 20.0f ? dt : (dt < -20.0f ? expf(dt) : log1pf(expf(dt)));
        const float g = -expf(a_log[vh]) * softplus_dt;
        const float decay = expf(g);
        const int state_size = k_dim * v_dim;
        ...
        for (int vd = tid; vd < v_dim; vd += blockDim.x) {
            const float gate = z_base[static_cast<size_t>(vh) * v_dim + vd];
            const float silu_gate = gate / (1.0f + expf(-gate));
            gated_row[static_cast<size_t>(vh) * v_dim + vd] = norm_weight[vd] * core[vd] * norm_scale * silu_gate;
        }
        __syncthreads();
    }
}
...
__global__ void full_attention_q_batch_kernel(
    const float * q_and_gate,
    const uint16_t * q_norm_weight,
    float * q,
    float * gate,
    ...
    const int src = static_cast<size_t>(tok) * q_total * 2 + h * head_dim * 2;
    const int dst = static_cast<size_t>(tok) * q_total + h * head_dim;
    float ss = 0.0f;
    for (int d = tid; d < head_dim; d += blockDim.x) {
        const float v = q_and_gate[src + d];
        q[dst + d] = v;
...
__global__ void full_attention_kv_batch_kernel(
    const float * k_in,
    const float * v_in,
    const uint16_t * k_norm_weight,
    float * key_cache,
    ...
    const int base = static_cast<size_t>(tok) * kv_total + h * head_dim;
    float ss = 0.0f;
    for (int d = tid; d < head_dim; d += blockDim.x) {
        const float v = k_in[base + d];
        k_local[d] = v;
...
__global__ void full_attention_attend_batch_kernel(
    const float * q,
    const float * gate,
    const float * key_cache,
    const float * value_cache,
    ...
    extern __shared__ float shared[];
    float * scores = shared;
    float * partial = scores + start_pos + tokens;
    const int block = blockIdx.x;
    const int tok = block / n_heads;
    const int h = block - tok * n_heads;
    const int tid = threadIdx.x;
    ...
    const int pos = start_pos + tok;
    const float scale = rsqrtf(static_cast<float>(head_dim));
    const float * qh = q + static_cast<size_t>(tok) * q_total + h * head_dim;
    ...
    const float denom = partial[0];
    float * out = attn + static_cast<size_t>(tok) * q_total + h * head_dim;
    for (int d = tid; d < head_dim; d += blockDim.x) {
        float sum = 0.0f;
        for (int t = 0; t <= pos; ++t) {
            const float prob = scores[t] / denom;
        ...
        const float g = gate[static_cast<size_t>(tok) * q_total + h * head_dim + d];
        out[d] = sum * (1.0f / (1.0f + expf(-g)));
    }
    (void) max_seq_len;
}
...
__global__ void bf16_matvec_kernel(
    ...
    const uint16_t * w = weight + static_cast<size_t>(row) * cols;
    float sum = 0.0f;
    for (int col = tid; col < cols; col += blockDim.x) {
        sum += bf16_bits_to_float(w[col]) * bf16_bits_to_float(x[col]);
    }
    partial[tid] = sum;
    __syncthreads();
    for (int stride = blockDim.x / 2; stride > 0; stride >>= 1) {
...
__global__ void add_float_kernel(const float * a, const float * b, float * out, int n) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) {
        return;
    }
...
__global__ void add_rms_norm_batch_to_bf16_kernel(
    const float * a,
    const float * b,
    const uint16_t * weight,
    float * sum_out,
    ...
    const size_t base = static_cast<size_t>(tok) * hidden;
    float ss = 0.0f;
    for (int i = tid; i < hidden; i += blockDim.x) {
    ...
    for (int i = tid; i < hidden; i += blockDim.x) {
        const float w = bf16_bits_to_float(weight[i]);
        const float factor = one_plus ? (1.0f + w) : w;
        norm_out[base + i] = float_to_bf16_bits(sum_out[base + i] * scale * factor);
    }
}

__global__ void add_rms_norm_to_bf16_kernel(
    const float * a,
    const float * b,
    const uint16_t * weight,
    float * sum_out,
    ...
    for (int i = tid; i < n; i += blockDim.x) {
        const float w = bf16_bits_to_float(weight[i]);
        const float factor = one_plus ? (1.0f + w) : w;
        norm_out[i] = float_to_bf16_bits(sum_out[i] * scale * factor);
    }
}
...
void launch_silu_mul_gate_up_batch(const float * gate_up, float * out, int tokens, int intermediate, void * stream) {
    ...
    silu_mul_gate_up_batch_kernel<<<grid, block, 0, static_cast<cudaStream_t>(stream)>>>(gate_up, out, tokens, intermediate);
}
...
void launch_float_to_f16(const float * input, uint16_t * output, int n, void * stream) {
    const int block = 256;
    const int grid = (n + block - 1) / block;
    float_to_f16_kernel<<<grid, block, 0, static_cast<cudaStream_t>(stream)>>>(input, output, n);
}

void launch_lowp_row_to_float(const uint16_t * input, float * output, int n, int lowp_type, void * stream) {
    const int block = 256;
    const int grid = (n + block - 1) / block;
    lowp_row_to_float_kernel<<<grid, block, 0, static_cast<cudaStream_t>(stream)>>>(input, output, n, lowp_type);
}

void launch_lowp_embedding_id_to_float(
    const uint16_t * input,
    const int * token_id,
    float * output,
    int vocab,
    ...
    lowp_embedding_id_to_float_kernel<<<grid, block, 0, static_cast<cudaStream_t>(stream)>>>(
        input,
        token_id,
        output,
        vocab,
...
void launch_copy_int(const int * input, int * output, void * stream) {
    copy_int_kernel<<<1, 1, 0, static_cast<cudaStream_t>(stream)>>>(input, output);
}

void launch_embedding_batch_to_float(
    const uint16_t * emb,
    const int * token_ids,
    float * output,
    int tokens,
    ...
    void * stream) {
    const int block = 256;
    const int grid = (tokens * hidden + block - 1) / block;
    embedding_batch_to_float_kernel<<<grid, block, 0, static_cast<cudaStream_t>(stream)>>>(
        emb,
        token_ids,
        output,
        tokens,
...
void launch_rms_norm_batch_to_bf16(
    const float * input,
    const uint16_t * weight,
    uint16_t * output,
    int tokens,
    ...
    bool one_plus,
    void * stream) {
    rms_norm_batch_to_bf16_kernel<<<tokens, 256, 0, static_cast<cudaStream_t>(stream)>>>(
        input,
        weight,
        output,
        tokens,
...
void launch_rms_norm_to_f16(
    ...
    bool one_plus,
    void * stream) {
    rms_norm_to_f16_kernel<<<1, 256, 0, static_cast<cudaStream_t>(stream)>>>(input, weight, output, n, eps, one_plus);
}
...
void launch_linear_attention_conv_batch(
    ...
    const int block = 256;
    const int grid = (conv_dim + block - 1) / block;
    linear_attention_conv_batch_kernel<<<grid, block, 0, static_cast<cudaStream_t>(stream)>>>(
        mixed,
        conv_weight,
        conv_state,
        conv_out,
...
void launch_linear_attention_recurrent_batch(
    ...
    const size_t shared_bytes =
        static_cast<size_t>(k_dim * 2 + v_dim * 2 + block * 2) * sizeof(float);
    linear_attention_recurrent_batch_kernel<<<value_heads, block, shared_bytes, static_cast<cudaStream_t>(stream)>>>(
        conv_out,
        z,
        b,
        a,
...
void launch_full_attention_q_batch(
    ...
    float eps,
    void * stream) {
    full_attention_q_batch_kernel<<<tokens * n_heads, 256, 0, static_cast<cudaStream_t>(stream)>>>(
        q_and_gate,
        q_norm_weight,
        q,
        gate,
...
void launch_full_attention_kv_batch(
    ...
    float eps,
    void * stream) {
    full_attention_kv_batch_kernel<<<tokens * kv_heads, 256, 0, static_cast<cudaStream_t>(stream)>>>(
        k_in,
        v_in,
        k_norm_weight,
        key_cache,
...
void launch_full_attention_attend_batch(
    ...
    int start_pos,
    void * stream) {
    const size_t shared_bytes = static_cast<size_t>(start_pos + tokens + 256) * sizeof(float);
    full_attention_attend_batch_kernel<<<tokens * n_heads, 256, shared_bytes, static_cast<cudaStream_t>(stream)>>>(
        q,
        gate,
        key_cache,
        value_cache,
...
void launch_bf16_matvec(
    ...
    int cols,
    void * stream) {
    bf16_matvec_kernel<<<rows, 256, 0, static_cast<cudaStream_t>(stream)>>>(weight, x, y, rows, cols);
}

void launch_add_float(const float * a, const float * b, float * out, int n, void * stream) {
    const int block = 256;
    const int grid = (n + block - 1) / block;
    add_float_kernel<<<grid, block, 0, static_cast<cudaStream_t>(stream)>>>(a, b, out, n);
}

void launch_add_float_batch(const float * a, const float * b, float * out, int n, void * stream) {
    launch_add_float(a, b, out, n, stream);
}
...
void launch_add_rms_norm_batch_to_bf16(
    ...
    bool one_plus,
    void * stream) {
    add_rms_norm_batch_to_bf16_kernel<<<tokens, 256, 0, static_cast<cudaStream_t>(stream)>>>(
        a,
        b,
        weight,
        sum_out,
...
void launch_add_rms_norm_to_bf16(
    ...
    bool one_plus,
    void * stream) {
    add_rms_norm_to_bf16_kernel<<<1, 256, 0, static_cast<cudaStream_t>(stream)>>>(
        a,
        b,
        weight,
        sum_out,
