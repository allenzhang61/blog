...
void copy_params(GPTModel& dst, GPTModel& src) {
    ...
    check(dst_params.size() == src_params.size(), "copy params size");
    for (size_t i = 0; i < dst_params.size(); ++i) {
        check(dst_params[i]->shape() == src_params[i]->shape(), "copy params shape");
...
void test_metal_elementwise() {
    ...
    check_tensor_close(ops::add(a_metal, b_metal), ops::add(a_cpu, b_cpu), 1e-5, "metal add");
    check_tensor_close(ops::mul(a_metal, b_metal), ops::mul(a_cpu, b_cpu), 1e-5, "metal mul");
    check_tensor_close(ops::mul_scalar(a_metal, 0.5), ops::mul_scalar(a_cpu, 0.5), 1e-5, "metal mul_scalar");
}

void test_metal_matmul() {
    Device metal_device = Device::parse("metal");
    ...
    Tensor a_metal = Tensor::from_vector(a_cpu.data(), a_cpu.shape(), metal_device);
    Tensor b_metal = Tensor::from_vector(b_cpu.data(), b_cpu.shape(), metal_device);
    check_tensor_close(ops::matmul(a_metal, b_metal), ops::matmul(a_cpu, b_cpu), 1e-4, "metal matmul");
}

void test_metal_batch_matmul() {
    Device metal_device = Device::parse("metal");
    Tensor a_cpu = Tensor::from_vector({1, 2, 3, 4, 5, 6, 7, 8}, {1, 1, 2, 4});
    Tensor b_cpu = Tensor::from_vector({1, 0, 0, 1, 2, 1, 1, 2}, {1, 1, 4, 2});
    Tensor a_metal = Tensor::from_vector(a_cpu.data(), a_cpu.shape(), metal_device);
    Tensor b_metal = Tensor::from_vector(b_cpu.data(), b_cpu.shape(), metal_device);
    check_tensor_close(ops::batch_matmul(a_metal, b_metal), ops::batch_matmul(a_cpu, b_cpu), 1e-4, "metal batch_matmul");
}

void test_metal_softmax_layernorm_embedding_cross_entropy() {
    Device metal_device = Device::parse("metal");

    Tensor s_cpu = Tensor::from_vector({1, 2, 3, 1, 0, -1}, {2, 3});
    Tensor s_metal = Tensor::from_vector(s_cpu.data(), s_cpu.shape(), metal_device);
    check_tensor_close(ops::softmax(s_metal), ops::softmax(s_cpu), 1e-5, "metal softmax");
    ...
    check_tensor_close(ops::layernorm(x_metal, scale_metal, shift_metal), ops::layernorm(x_cpu, scale_cpu, shift_cpu),
    ...
    check_tensor_close(ops::embedding(ids_metal, weight_metal), ops::embedding(ids_cpu, weight_cpu), 1e-5,
                       "metal embedding");
    ...
    check_close(ops::cross_entropy(logits_metal, targets_metal).item(),
                ops::cross_entropy(logits_cpu, targets_cpu).item(),
                1e-5, "metal cross_entropy");
}

void test_metal_gelu() {
    Device metal_device = Device::parse("metal");
    Tensor x_cpu = Tensor::from_vector({-1.0, 0.0, 1.0, 2.0}, {4});
    Tensor x_metal = Tensor::from_vector(x_cpu.data(), x_cpu.shape(), metal_device);
    check_tensor_close(ops::gelu(x_metal), ops::gelu(x_cpu), 1e-5, "metal gelu");
}

void test_metal_model_subgraph() {
    Device metal_device = Device::parse("metal");
    ...
    check_tensor_close(gpu, cpu, 1e-4, "metal model subgraph");
}

void test_metal_gpt_forward_matches_cpu() {
    GPTConfig cfg;
    cfg.vocab_size = 32;
    ...
    check_tensor_close(metal_logits, cpu_logits, 2e-3, "metal gpt forward");
}

void test_metal_gpt_train_step() {
    GPTConfig cfg;
    cfg.vocab_size = 32;
    cfg.context_length = 4;
    cfg.emb_dim = 8;
    ...
    check(std::isfinite(loss) && loss > 0.0, "metal train loss finite");
}
...
int main() {
    try {
        ...
        test_metal_elementwise();
        test_metal_matmul();
        test_metal_batch_matmul();
        test_metal_softmax_layernorm_embedding_cross_entropy();
        test_metal_gelu();
        test_metal_model_subgraph();
        test_metal_gpt_forward_matches_cpu();
        test_metal_gpt_train_step();
        std::cout << "all llm-train-cpp Metal smoke tests passed\n";
