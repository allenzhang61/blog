...
public:
    ...
    bool available() const { return device_ != nil && queue_ != nil && library_ != nil; }

    std::string status() const {
        if (available()) return "Metal backend available";
        return status_;
    }
    ...
    std::vector<float> run1d(const char* kernel_name,
                             ...
                             uint32_t b_size,
                             int64_t count) {
        if (!available()) throw std::runtime_error(status_);
        if (count <= 0) return {};
...
private:
    MetalRuntime() {
        @autoreleasepool {
            ...
            if (library_ == nil) {
                ...
                if (error != nil) status_ += ": " + std::string([[error localizedDescription] UTF8String]);
                return;
            }
    ...
    id<MTLComputePipelineState> get_pipeline(const char* name) {
        ...
        if (function == nil) throw std::runtime_error("Metal function not found: " + key);
        NSError* error = nil;
        ...
        if (pipeline == nil) {
            ...
            if (error != nil) message += ": " + std::string([[error localizedDescription] UTF8String]);
...
Tensor mul(const Tensor& a, const Tensor& b) {
    if (a.shape() != b.shape()) throw std::runtime_error("Metal mul expects same shape");
...
Tensor batch_matmul(const Tensor& a, const Tensor& b) {
    if (a.shape().size() != 4 || b.shape().size() != 4) throw std::runtime_error("Metal batch_matmul expects 4D tensors");
    ...
    if (b.shape()[0] != B || b.shape()[1] != H || b.shape()[2] != K) throw std::runtime_error("Metal batch_matmul shape mismatch");
...
Tensor softmax(const Tensor& a, int64_t dim) {
    ...
    if (dim != rank - 1) throw std::runtime_error("Metal softmax currently supports last dim only");
...
Tensor layernorm(const Tensor& x, const Tensor& scale, const Tensor& shift, double eps) {
    ...
    for (int64_t r = 0; r < rows; ++r) {
        ...
        for (int64_t c = 0; c < C; ++c) xhat[r * C + c] = (x.data()[r * C + c] - mean) * invs[r];
    }
...
Tensor embedding(const Tensor& ids, const Tensor& weight) {
    if (weight.shape().size() != 2) throw std::runtime_error("Metal embedding weight must be 2D");
    ...
    if (weight.requires_grad()) {
        ...
        out.node->backward_fn = [ids, weight, out, dim]() mutable {
            for (int64_t i = 0; i < ids.numel(); ++i) {
                ...
                for (int64_t d = 0; d < dim; ++d) weight.grad()[id * dim + d] += out.grad()[i * dim + d];
            }
        };
    }
    return out;
...
Tensor cross_entropy(const Tensor& logits, const Tensor& targets) {
    if (logits.shape().size() != 3) throw std::runtime_error("Metal cross_entropy expects logits [B,T,V]");
    int64_t B = logits.shape()[0], T = logits.shape()[1], V = logits.shape()[2];
    if (targets.numel() != B * T) throw std::runtime_error("Metal cross_entropy target shape mismatch");
    ...
    double loss = 0.0;
    for (auto value : row_losses) loss += value;
