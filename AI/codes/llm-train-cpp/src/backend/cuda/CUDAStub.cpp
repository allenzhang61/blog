...
bool cuda_backend_available() {
    ...
    // -> src/kernels/cuda/cuda_ops.cu::llm::cuda::available()
    return false;
}
...
std::string cuda_backend_status() {
    ...
    // -> src/kernels/cuda/cuda_ops.cu::llm::cuda::status()
...
Tensor add(const Tensor&, const Tensor&) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::add()
...
Tensor sub(const Tensor&, const Tensor&) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::sub()
...
Tensor mul(const Tensor&, const Tensor&) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::mul() -> mul_kernel()
    throw std::runtime_error(status());
}

Tensor div(const Tensor&, const Tensor&) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::div() -> div_kernel()
    throw std::runtime_error(status());
}

Tensor mul_scalar(const Tensor&, double) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::mul_scalar() -> mul_scalar_kernel()
    throw std::runtime_error(status());
}

Tensor pow(const Tensor&, double) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::pow() -> pow_kernel()
    throw std::runtime_error(status());
}

Tensor sum(const Tensor&) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::sum() -> sum_kernel()
    throw std::runtime_error(status());
}

Tensor mean(const Tensor&) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::mean() -> 先调用 sum_kernel()，均值除法在 host 侧完成。
    throw std::runtime_error(status());
}

Tensor max(const Tensor&) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::max() -> max_kernel()
    throw std::runtime_error(status());
}

Tensor reshape(const Tensor&, const std::vector<int64_t>&) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::reshape() -> copy_kernel()
    throw std::runtime_error(status());
}

Tensor transpose(const Tensor&, int64_t, int64_t) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::transpose() -> gather_kernel()
    throw std::runtime_error(status());
}

Tensor matmul(const Tensor&, const Tensor&) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::matmul() -> matmul_kernel()
    throw std::runtime_error(status());
}

Tensor batch_matmul(const Tensor&, const Tensor&) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::batch_matmul() -> batch_matmul_kernel()
    throw std::runtime_error(status());
}

Tensor causal_mask(const Tensor&, int64_t, double) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::causal_mask() -> causal_mask_kernel()
    throw std::runtime_error(status());
}

Tensor softmax(const Tensor&, int64_t) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::softmax() -> softmax_kernel()
    throw std::runtime_error(status());
}

Tensor log_softmax(const Tensor&, int64_t) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::log_softmax() -> softmax_kernel() -> log_kernel()
    throw std::runtime_error(status());
}

Tensor cross_entropy(const Tensor&, const Tensor&) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::cross_entropy() -> cross_entropy_row_loss_kernel()
    throw std::runtime_error(status());
}

Tensor embedding(const Tensor&, const Tensor&) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::embedding() -> embedding_kernel()
    throw std::runtime_error(status());
}

Tensor layernorm(const Tensor&, const Tensor&, const Tensor&, double) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::layernorm() -> layernorm_kernel()
    throw std::runtime_error(status());
}

Tensor gelu(const Tensor&) {
    // CUDA 开启时对应：
    // src/kernels/cuda/cuda_ops.cu::llm::cuda::gelu() -> gelu_kernel()
    throw std::runtime_error(status());
}
