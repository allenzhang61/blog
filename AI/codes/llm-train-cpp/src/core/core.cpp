...
Device Device::parse(const std::string& text) {
    if (text == "cpu") return {DeviceType::CPU, 0};
    if (text == "cuda" || text == "cuda:0") return {DeviceType::CUDA, 0};
    if (text == "metal" || text == "metal:0") return {DeviceType::Metal, 0};
...
int64_t canonical_dim(int64_t dim, int64_t rank) {
    if (dim < 0) dim += rank;
    if (dim < 0 || dim >= rank) throw std::runtime_error("dim out of range");
    return dim;
}

void check(bool cond, const std::string& message) {
    if (!cond) throw std::runtime_error("check failed: " + message);
}
