...
Tensor softmax(const Tensor& a, int64_t dim) {
    ...
    if (dim != rank - 1) {
        throw std::runtime_error("softmax currently supports last dim only");
    }
    ...
    for (int64_t r = 0; r < rows; ++r) {
        ...
        for (int64_t c = 0; c < width; ++c) {
            mx = std::max(mx, a.data()[r * width + c]);
        }
        double denom = 0.0;
        for (int64_t c = 0; c < width; ++c) {
            denom += std::exp(a.data()[r * width + c] - mx);
        }
        for (int64_t c = 0; c < width; ++c) {
            out.data()[r * width + c] = std::exp(a.data()[r * width + c] - mx) / denom;
        }
    }
    if (a.requires_grad()) {
        out.node->parents = {a};
...
Tensor log_softmax(const Tensor& a, int64_t dim) {
    ...
    for (int64_t i = 0; i < out.numel(); ++i) {
        out.data()[i] = std::log(std::max(s.data()[i], 1e-12));
    }
    return out;
}
...
Tensor cross_entropy(const Tensor& logits, const Tensor& targets) {
    ...
    if (targets.numel() != B * T) {
        throw std::runtime_error("cross_entropy target shape mismatch");
    }
